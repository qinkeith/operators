...
1. Creating the project (ensure operator-sdk is in your PATH)
...
```sh
make docker-build docker-push IMG=<some-registry>/clair-operator:tag
```
	
3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/clair-operator:tag
```
