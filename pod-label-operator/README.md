...
- Initialize the project
  ...
  ```bash
  mkdir pod-label-operator

  cd pod-label-operator
...
```sh
make docker-build docker-push IMG=<some-registry>/pod-label-operator:tag
```
	
3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/pod-label-operator:tag
```
